...
export const UPDATE_ADMIN_CUSTOMER_PAYLOAD = joi.object().keys({ customerId: joi.string().trim().guid({ version: ["uuidv1"] }).required(), password: joi.string().trim().min(4).max(8), name: joi.string().trim(), mobile: joi.string().trim().regex(/^[0-9]+/), email: joi.string().trim(), amount: joi.string().trim().regex(/^[0-9]{1,15}(\.[0-9]{1,8})?$/), location: joi.string().trim(), info: joi.string().trim(), handler: joi.string().trim(), start: joi.date(), end: joi.date(), allowed: joi.string().trim(), limit: joi.string().trim(), freeze: joi.boolean() });
