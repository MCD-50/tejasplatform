...
// CUSTOMER
export const CREATE_ADMIN_CUSTOMER_PAYLOAD = joi.object().keys({ userId: joi.string().trim().min(4).max(12).required(), password: joi.string().trim().min(4).max(8).required(), type: joi.string().trim().valid(["user", "admin"]), name: joi.string().trim(), mobile: joi.string().trim().regex(/^[0-9]+/), email: joi.string().trim(), amount: joi.string().trim().regex(/^[0-9]{1,15}(\.[0-9]{1,8})?$/), location: joi.string().trim(), handler: joi.string().trim(), start: joi.date().required(), end: joi.date().required(), allowed: joi.string().trim(), limit: joi.string().trim(), device: joi.string().trim().valid(["mobile", "web", "all"]) });
