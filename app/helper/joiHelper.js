...
// CUSTOMER
export const CREATE_ADMIN_CUSTOMER_PAYLOAD = joi.object().keys({ password: joi.string().trim().required(), type: joi.string().trim().valid(["user", "admin"]), name: joi.string().trim(), mobile: joi.string().trim(), email: joi.string().trim(), amount: joi.string().trim(), location: joi.string().trim(), handler: joi.string().trim(), start: joi.date().required(), end: joi.date().required(), allowed: joi.string().trim(), limit: joi.number() });
export const LOGIN_PAYLOAD = joi.object().keys({ userId: joi.string().trim().length(10).required(), password: joi.string().trim().required() });
export const UPDATE_ADMIN_CUSTOMER_PAYLOAD = joi.object().keys({ customerId: joi.string().trim().guid({ version: ["uuidv1"] }).required(), password: joi.string().trim(), name: joi.string().trim(), mobile: joi.string().trim(), email: joi.string().trim(), amount: joi.string().trim(), location: joi.string().trim(), handler: joi.string().trim(), start: joi.date(), end: joi.date(), allowed: joi.string().trim(), limit: joi.number(), freeze: joi.boolean() });
...
// MARKET => mongo
export const CREATE_ADMIN_PLAN_PAYLOAD = joi.object().keys({ name: joi.string().trim().required(), price: joi.number().required() });
