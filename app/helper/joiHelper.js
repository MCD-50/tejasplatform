...
export const GET_ADMIN_CUSTOMERS_PAYLOAD = joi.object().keys({ customerId: joi.string().trim().guid({ version: ["uuidv1"] }), userId: joi.string().trim().min(4).max(12), type: joi.string().trim().valid(["user", "admin"]), location: joi.string().trim(), name: joi.string().trim(), mobile: joi.string().trim(), email: joi.string().trim(), allowed: joi.string().trim(), handler: joi.string().trim(), page: joi.number().default(1), limit: joi.number().default(20) });
