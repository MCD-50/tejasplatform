...
export const GET_ADMIN_MARKETS_PAYLOAD = joi.object().keys({ customerId: joi.string().trim().guid({ version: ["uuidv1"] }), market: joi.string().trim().valid(constant.setting.meta.markets), target: joi.string().trim().valid(constant.setting.meta.targets), page: joi.number().default(1), limit: joi.number().default(20) });
...
export const GET_ADMIN_CUSTOMERS_PAYLOAD = joi.object().keys({ customerId: joi.string().trim().guid({ version: ["uuidv1"] }), userId: joi.string().trim().length(10), type: joi.string().trim().valid(["user", "admin"]), page: joi.number().default(1), limit: joi.number().default(20) });
