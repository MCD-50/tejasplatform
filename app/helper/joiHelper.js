...
export const UPDATE_ADMIN_CUSTOMER_PAYLOAD = joi.object().keys({ customerId: joi.string().trim().guid({ version: ["uuidv1"] }).required(), password: joi.string().trim().min(4).max(8), name: joi.string().trim(), mobile: joi.string().trim(), email: joi.string().trim(), amount: joi.string().trim(), location: joi.string().trim(), handler: joi.string().trim(), start: joi.date(), end: joi.date(), allowed: joi.string().trim(), limit: joi.number(), freeze: joi.boolean() });
