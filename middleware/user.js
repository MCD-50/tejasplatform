...
export const requestCheck = async (req, res, next) => {
	try {
		...
		if (!jwtverified || !jwtverified.customerId || !jwtverified.device || jwtverified.type != "user") return res.status(401).json(collection.getJsonError({ error: "Something went wrong" }));
		...
		if (!jwtverified.end || moment(jwtverified.end).isBefore(moment())) {
